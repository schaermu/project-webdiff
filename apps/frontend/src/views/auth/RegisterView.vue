...
<template>
    <div class="mt-10 mx-auto sm:w-full sm:max-w-sm">
        ...
        <Form @submit="onSubmit" :validation-schema="schema">
            ...
            <Input class="my-5" name="emailRepeat" label="Repeat E-Mail Address" />
...
const schema = yup.object({
    ...
    emailRepeat: yup.string().required('E-Mail Address is required').email('E-Mail Adresses must match'),
...
function onSubmit(values, actions) {
    ...
    authStore.register(values.email, values.password, values.passwordRepeat, captcha.value).then((result) => {
        if (result.ok) {
            resetForm()
